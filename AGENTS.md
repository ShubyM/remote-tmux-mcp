...
- Each command gets its own tmux window/tab, not a split pane.
...
- Use `tmux_session_snapshot` and `tmux_capture_pane` as the invisible observation layer over the managed tmux session.
...
3. Keep separate tasks in separate tmux windows/tabs.
